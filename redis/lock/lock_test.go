...
func TestExtendLock(t *testing.T) {
	...
	var GetRandomKey = func(key string) string {
		return fmt.Sprintf("test-extend:%s:%d", key, createdDate)
	}
	...
	for _, item := range tests {
		t.Run(item.name, func(t *testing.T) {
			...
			// 测试需要加锁
			if item.isTryLock {
				isGetLock, err := redis.TryLock(ctx, item.key, item.Value, time.Second)
				t.Log(err)
				assert.Equal(t, item.isLockSuccess, isGetLock)
			}
			...
			go func() {
				for i := 0; i < 2; i++ {
					...
					t.Log(redis.ExtendLock(ctx, item.key, item.Value, time.Second*2))
				}
			}()
