...
1、node1 中的 container-1 容器访问 node2 的容器 container-2，flanneld 在收到 container-1 发给 container-2 的 IP 包之后，就会把这个 IP 包直接封装在一个 UDP 包里，然后发送给 Node2，这个 UDP 包的源地址就是 就是 flanneld 所在的 Node1 的地址，目的地址就是 container-2 所在的宿主机 Node2 的地址；   

2、每台宿主机上的 flanneld，都监听着一个 8285 端口，只要把 UDP 数据发送到节点中的 8285 端口即可；  

3、宿主机之间通过 UDP 通信，就能将数据发送到对方，节点中监听 8285 端口的进程 flanneld，就能从 UDP 数据包中解析出容器的原 IP 数据包；  

4、flanneld 会把接收到的 IP 包，交给 flannel0 中，flannel0 是一个 TUN 设备，Flannel 进程向 flannel0 的数据包，会从用户态转向内核态，交给 Linux 内核网络栈处理；  

5、Linux 内核网络栈，会根据本机的路由决定 IP 数据包的下一步流向，这个数据包按照规则会被转发到 docker0 网桥；   

6、这样 docker0 网桥会扮演二层交换机的角色，配合 `Veth Pair`，就能找到对应的容器，进行数据的发送。      
...
2、VTEP 设备 设备收到发送端的原始 IP 后，就会加上一个目的 MAC 地址，封装成一个二层数据帧，然后发送给“目的 VTEP 设备”；   
