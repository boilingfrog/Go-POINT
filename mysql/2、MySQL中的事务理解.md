...
<img src="/img/mysql/mysql-next-key-demo3.png"  alt="mysql" />    

因为 事务1 在 事务2 提交插入操作之后，使用了 `for updated` 当前读，这就会出现读取的结果和第一次读取结果的不一致。   

**场景2**  

<img src="/img/mysql/mysql-next-key-demo4.png"  alt="mysql" />    

类似于场景1，因为事务1，在事务2进行插入数据之后，执行了数据更新的操作，数据更新的操作会先查后修改，这个查询就是当前读，所以找到找到刚插入的数据并且修改，这时候当前事务中的 trx_id 就是最新的了，后面的查询就能读取到刚刚更新的数据。   
