...
10、如果`old state`的状态是未被锁状态，并且锁不处于饥饿状态,那么当前`goroutine`已经获取了锁的拥有权，结束Locak;  

11、判断一下当前`goroutine`是新来的还是刚被唤醒的，新来的加入到等待队列的尾部，刚被唤醒的加入到等待队列的头部；  

12、判断如果当前`state`是否是饥饿状态，不是的唤醒本次`goroutine`,继续循环，是饥饿状态继续往下面走；  

13、当前`goroutine`来设置锁，等待者减一，获取锁。   
